  ...
  void ParseCommandLine(int argc, char **argv) {
    ...
    while (1) {
      static struct option long_options[] = {
          ...
          {0, 0, 0, 0}};
      int option_index = 0;
      int c = getopt_long(argc, argv, "h:n:m:t:p:d:o:?", long_options,
                          &option_index);
      if (c == -1)
        break;
      switch (c) {
...
extern "C" int GetIPList(void *thread, addr_t *addr, int max, int ipv4,
                         int ipv6);
...
public:
  ...
  void cacheHit(bool force = false) {
    ...
    cacheHits++;
    if (force || cacheHits > (cache.size() * cache.size() / 400) ||
        (cacheHits * cacheHits > cache.size() / 20 && (now - cacheTime > 5))) {
      set<CNetAddr> ips;
...
extern "C" int GetIPList(void *data, addr_t *addr, int max, int ipv4,
                         int ipv6) {
...
static const string mainnet_seeds[] = {"seed.galaxy-hub.online",
                                       "seed2.galaxy-hub.online", ""};

static const string testnet_seeds[] = {"tseed.galaxy-hub.online",
                                       "tseed2.galaxy-hub.online", ""};
